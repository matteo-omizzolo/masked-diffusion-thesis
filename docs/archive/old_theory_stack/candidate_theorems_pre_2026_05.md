> ARCHIVED — historical only. April 2026 theorem stack (Phase 3b vintage).
> Do not use this file to infer current thesis status.
> The active theorem stack is `research/candidate_theorems.md` §0–§7.
>
> Status of objects in this file (May 2026):
> - "Refinement A′" (variance form) — superseded by Diagnostic A′ (additivity scale, not a regret refinement).
> - "Refinement A″" (rank form) — superseded by Diagnostic A″ (rankability, not a regret refinement).
> - "Negative-Result Corollary" — superseded by the Empirical Ranker-Class Limitation (formal time-only part + scoped empirical part).
> - "Proposition C" (γ B(B−1)/2 pairwise interaction bound) — superseded by Theorem B / B′ + Diagnostic Framework C.
> - "Theorem A-ad" (adaptive/state-conditional) — Protocol C honest negative; Appendix F only.
>
> Retained here as proof provenance and for citation. Git history preserves the
...
**Historical status claimed in April 2026 (superseded):** Theorem A +
Refinements A′/A″ + Negative-Result Corollary were presented as formally
proved under explicit assumptions. This is **not** current status; see the
archive banner above and `research/candidate_theorems.md` §0–§7.
