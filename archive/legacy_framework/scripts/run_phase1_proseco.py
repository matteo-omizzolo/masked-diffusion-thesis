#!/usr/bin/env python3
"""Phase 1 Experiment — ProSeCo Backend: Protocol A + B.

Primary empirical platform for the thesis question:
  "For a fixed predictor schedule and fixed corrector NFE budget in masked
  diffusion language models, can aggregate trajectory signals predict the
  marginal value of a corrective refinement loop well enough to outperform
  uniform corrector placement?"

This script uses the ProSeCo-style annealed-refinement corrector (not the MDLM
heuristic Gibbs resample). The corrector refines UNMASKED (committed) tokens
by running corrector_steps backbone calls at decreasing noise levels.

Usage:
    # Surrogate (pipeline validation, CPU-only)
    python scripts/run_phase1_proseco.py --surrogate \
        --T 64 --N 10 --M 8 --P 50 --B_values 4,8,16 --seed 42

    # Real ProSeCo corrector on MDLM checkpoint (HPC, requires GPU)
...
def main():
    ...
    run_config = {
        "backend": "surrogate" if args.surrogate else "proseco",
    ...
    print("=" * 60)
    print("Phase 1 ProSeCo Experiment")
    print("=" * 60)
    print(f"  Backend:  {'SURROGATE' if args.surrogate else 'PROSECO-CORRECTOR (MDLM backbone)'}")
    ...
    if args.surrogate:
        ...
        try:
            ...
            print(f"ERROR: Could not load ProSeCo backend: {e}")
